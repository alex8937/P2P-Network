...
//      System.out.println("File received" + (passedlen * 100 / len) + "%");   
    ...
    public static void main(String[] args)throws IOException {
		...
		String numofile = new Client1().receivemsg("localhost", Integer.parseInt(serverport)+1);
		
		String numfromserver = new Client1().receivemsg("localhost", Integer.parseInt(serverport)+11);
		String Filename = new Client1().receivemsg("localhost", Integer.parseInt(serverport)+21);
		...
		for(int i = 1; i< = Integer.parseInt(numfromserver); i++){		
			new Client1().receiveFile(dir+"/client1/", "localhost", Integer.parseInt(serverport)+31);		
		}
		Thread snd = new Thread(new MyRunnable_snd(1,Integer.parseInt(c1to),Filename,nfile,Integer.parseInt(clinetport1)));
		Thread rec = new Thread(new MyRunnable_rec(c1from,Filename,nfile,Integer.parseInt(recport1)));
		snd.start();		
		rec.start();

		//File file0 = new File("C:/Users/Ultra-seven/Desktop/Java/client1/a.pdf.001");
		//File file1 = new File("C:/Users/Ultra-seven/Desktop/Java/client1/a.pdf");
		//new Client1().mergeFiles(file0, file1);
      }
		...
		public void run() {
			...
			while(rest_snd!= 0){
				...
				for(int j = 1;j< = numofile;j++){
					Path path1 = Paths.get(dir+"/client1/"+Filename+"."+String.format("%03d",j));	
...
//							System.out.println(j);
							try{
							new Client1().sendFile00(dir+"/client1/"+Filename+"."+String.format("%03d",j),sock);
...
//					System.out.println("Connected client");	
					new Client1().receiveFile00(dir+"/client1/",clientSock);	
//					System.out.println("received");	
			...
			while(rest_rec =  = 0){
				try{
					...
					File file0 = new File(dir+"/client1/"+Filename+".001");
					File file1 = new File(dir+"/client1/"+Filename);
					new Client1().mergeFiles(file0, file1);
